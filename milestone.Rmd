...
Read in each of the subsample txts. Use the `tm` text mining package in R to clean and analyze the subsample. 
...
* Single quotes and apostrophes: I choose to keep intra-word apostrophes, to distinguish between e.g. `its` and `it's`. Though I would like to keep leading apostrophes as well, I can't, because it's difficult to distinguish between those and the start of a sentence. So, for example, my code will leave `can't` unchanged, but will turn `'hello world'` into `hello world`, and similarly will turn `'twas` into `twas`. 
...
For this we can again use the `tm` package.
...
Example of how to navigate the TDM: Let's look for the term "winter", plus the 4 terms that come after alphabetically.
...
Make a histogram of the frequency counts. The TDMs are extremely sparse, and also large. As a result, when I tried to do a straightforward `rowSums(as.matrix(TDM))`, I got an integer overflow error. To get around that, I use the `row_sums()` function from the `slam` package, which is meant to handle sparse arrays.
...
Clearly, there are truckloads of words that only appear once. We can extract some random examples.

```{r once}
blog.once <- findFreqTerms(blog.tdm, lowfreq=0, highfreq=1)
length(blog.once)  # number of terms that appear at most one time
set.seed(773)
sample(blog.once, 5)  # random five
```

I can remove sparse terms so that the frequency histogram looks slightly less skewed.

```{r denser}
maxsparse <- 0.9  # set max empty space at 90% 
blog.tdm.denser <- removeSparseTerms(blog.tdm, maxsparse)
...
As expected, these are very common words. If we want to, it's possible to remove common words from the corpus, using `corpus <- tm_map(corpus, removeWords, stopwords("english"))` during the cleaning stage. I haven't done that because I'm not yet sure what the impact on the prediction algorithm will be.

### Word associations

We can find out which words are associated with which. Using the previous example for "winter":

```{r assoc}
winterwords <- findAssocs(blog.tdm, "winter", 0.1)  # min correlation = 0.1
winterwords
```
