...
We try to put each event type into one of the 48 buckets, and keep track of that grouping in a new column, `$EventType`. For those events that don't fit any bucket, we will leave the value as `NA`.
```{r}
...
I make this a bit easier by writing a helper function called `groupEv()`, which takes as arguments a list (one of the 48 lists above) and the corresponding event group name. It will fill in the group name to the appropriate rows in the new column.
...
One by one, we call the `groupEv()` function defined above, for each of the 48 groups identified.
...
We can check how many event types we managed to categorize.
```{r}
unplaced <- subset(df, is.na(df$EventType))
# Compute percentage of events we did not manage to categorize:
nrow(unplaced)/nrow(df)
```

Now that most of the event types have been put into one of 48 buckets, we convert the character column into a factor column.
```{r}
...
str(df$CROPDMGEXP)
```
For the purposes of this analysis, I shall assume that numerical values in the `DMGEXP` columns refer to exponents of 10, e.g. "2" would denote "10^2". I assume "h" refers to hundreds. For convenience, I assume that null, "+", "?", and "-" indicate "0". Following these assumptions, we rename the factor levels.
...
After renaming the factor levels, we can start to compute the total damage estimates to property and crops. For this we use `mutate()` from `library(dplyr)`.
...
We can further `summarize()` the data, such that fatalities, injuries, property damage and crop damage are all grouped by event type. 
...
#### Which weather event is worst for health?

We determine that ??? is the worst for health.

To examine the health consequences of weather events, we can make a stacked bar plot of total deaths and total injuries.
...
#### Which weather event has the worst economic consequences?

We determine that ?? have the worst economic consequences in terms of monetary damage to property and crops.

To examine the economic consequences, we can again use a stacked bar plot in a process similar to the one above.
...
We can also take a look to see whether health consequences are correlated with economic damage.
